...
# Description: Building CNN for glasses vs no glasses binary classification and running prediction.
...
if __name__ == '__main__':
    if os.path.exists('traindata2.npy'):
        train_data = np.load('traindata2.npy')
    else:
    ...
    with open('Task2c.csv', mode='w') as Task2c: # Writing to csv file 
        TaskWriter = csv.writer(Task2c, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        ...
        for data in test_data: # Run prediction for each image and store results
            ...
            if np.argmax(model_out) == 0: # [1,0] = smile, argmax should be for first element
                img_pred = 1
                print('Glasses Predicted')
                print('Argmax is ', np.argmax(model_out))
            else:
                img_pred = 0
                print('No glasses predicted')
                print('Argmax is ', np.argmax(model_out))
